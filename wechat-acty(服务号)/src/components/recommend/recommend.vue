<template>
  <div class="container">
    <!-- <div class="top-tip" @click="chooseVideo">{{topTips}}</div> -->
    <scroll class="scroll"
            :pullup="pullup"
            @scrollEnd="scrollEnd"
            :data="list">
    ...
    <div class="tabbar" v-show="false">
      <div class="tab-item" @click="toMyHome">
        <img src="./myhome-icon.png" alt="" class="icon">
        <span class="name">我的视频</span>
      </div>
      <div class="tab-item">
        <input type="file" ref="upload" class="upload" multiple>
        <img src="./upvideo-icon.png" alt="" class="icon">
        <span class="name">上传视频</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    scrollEnd() {
      ...
      this.page ++
      this.loading = true
      getRecommend(this.page, this.perpage, this.secdata).then(res => {
    ...
    _getRecommend() {
      this.hasMore = true
      getRecommend(this.page, this.perpage, this.secdata, this._shareid).then(res => {
        const result = JSON.parse(res)
        // console.log(result)
        if(result.result === 1) {
    ...
    // 提交评论接口
    _sendComment(item) {
      this.$fly.get(`${process.env.API_HOST}index.php?m=wechatService&c=share&a=addComment`, {
        vid: item.id,
    ...
    _refreshComment(item) {
      this.$fly.get(`${process.env.API_HOST}index.php?m=wechatService&c=share&a=commentList`, {
        vid: item.id,
        _secdata: this.secdata
      }).then(result => {
        const res = JSON.parse(result)
  ...
  mounted() {
    ...
    wx.ready(() => {
      wx.onMenuShareAppMessage({
        ...
        link: `http://wechat.qianqiana.com/index.php?m=wechatService&c=weixin&a=location_mv&shareid=${this.shareid}`,
